...
const mockDir = path.resolve(process.env.ITSLEARNING_MOCK_DIR ?? "../itslearning-mock-api");
const port = 4899;
...
test.skip(!existsSync(path.join(mockDir, "package.json")), "itslearning-mock-api not found (set ITSLEARNING_MOCK_DIR)");

test.beforeAll(async () => {
	mock = spawn(process.platform === "win32" ? "npm.cmd" : "npm", ["start"], {
		cwd: mockDir,
		shell: process.platform === "win32",
		env: { ...process.env, ITSLEARNING_MOCK_PORT: String(port), ITSLEARNING_MOCK_TRACE: "0" },
		stdio: "ignore",
	});
	for (let i = 0; i < 60; i++) {
		try {
...
test.afterAll(async () => {
	...
	if (mock?.pid) {
		if (process.platform === "win32") spawn("taskkill", ["/pid", String(mock.pid), "/t", "/f"]);
		else mock.kill();
	}
});
...
test("signs in through the mock and reaches the main window without CSP violations", async () => {
	...
	app = await electron.launch({
		...
		env: { ...process.env, ITSLEARNING_MOCK_URL: mockUrl, VITE_DEV_SERVER_URL: "" },
	});

	const consoleErrors: string[] = [];
	app.on("window", (page) => {
		page.on("console", (m) => {
			if (m.type() === "error" || m.type() === "warning") consoleErrors.push(`[${page.url()}] ${m.text()}`);
		});
	});

	// 1) login window (organisation picker)
	const login = await app.firstWindow();
	await expect(login.getByText("Mock University").first()).toBeVisible({ timeout: 30_000 });
	...
	await expect
		.poll(async () => (await app!.evaluate(({ BrowserWindow }) => BrowserWindow.getAllWindows().map((w) => w.webContents.getURL()))).some((u) => u.endsWith("/index.html") || u.includes("index.html")), { timeout: 60_000 })
		.toBe(true);
