...
function TitlebarButton({children, onClick, className}: {
    ...
    return (
        <button
            className={cn("w-6 h-6 rounded-full bg-foreground/10 inline-flex items-center justify-center p-0.5 hover:scale-110 transition-all hover:text-white", className)}
            onClick={onClick}
        >
            {children}
        </button>
