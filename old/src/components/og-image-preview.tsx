...
export default function OgImagePreview({url}: { url: string }) {
    ...
    if (isLoading) {
        return (
            <div className={"w-full max-w-lg space-y-2 animate-pulse"}>
                <div
                    className="flex h-64 items-center justify-center rounded bg-gray-300 dark:bg-gray-700">
    ...
    return (
        <a
            className={"my-6 w-full h-full max-w-lg flex-wrap border-neutral-300 dark:border-neutral-500 border rounded-lg overflow-clip shadow-xl"}
            href={data.url}
            target={"_blank"}
        ...
        >
            ...
            <div className={"p-3 text-left break-all"}>
                <h1 className={"sm:text-xl text-base font-bold text-center mb-2 truncate"}>{data.title}</h1>
                <h3 className={"sm:text-base text-sm truncate"}>{data.description}</h3>
                <span
                    className={"underline text-blue-500 text-xs"}>{data.url}</span>
            </div>
        </a>
    )
}