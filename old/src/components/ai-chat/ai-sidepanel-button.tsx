...
export default function AISidepanelButton({className, variant, ...props}: {
    className?: string,
    variant?: ButtonProps['variant']
} & Omit<ButtonProps, 'variant'>) {
    const {aiSidepanel, toggleSidebar} = useAISidepanel()
    return (
        <Button className={cn("group", className)} variant={variant ?? 'ghost'} {...props} data-active={aiSidepanel}
