...
export default function MessageTitleForm({
	...
	if (!isLoading) {
		currentThreadName =
			...
			messages!
				...
				?.Participants.filter((participant) => participant.PersonId !== user!.PersonId)
	...
	const MessagesHeaderTitleString = () => {
		...
		if (isChatNew && recipientsSelected)
			return 'New chat with ' + recipientsSelected.map((recipient) => recipient.Label).join(', ')
	...
	return (
		<>
			...
			{!isChatNew && !isChatUndefined && isSettingNewThreadName ? (
				...
				<h2 className='text-lg font-semibold truncate'>{MessagesHeaderTitleString()}</h2>
			)}
		</>
	)
}
