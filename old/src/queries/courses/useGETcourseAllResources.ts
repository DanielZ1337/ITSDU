...
export default function useGETcourseAllResources(courseId: number, queryConfig?: UseQueryOptions<ItslearningRestApiEntitiesPersonalCourseCourseResource[], Error, ItslearningRestApiEntitiesPersonalCourseCourseResource[], string[]>) {
    ...
    return useQuery([TanstackKeys.CourseAllResources, String(courseId)], async () => {
...
export async function getCourseAllResources(courseId: number): Promise<ItslearningRestApiEntitiesPersonalCourseCourseResource[]> {
    const folder_map = new Map<number, ItslearningRestApiEntitiesPersonalCourseCourseResource>();
    ...
    const getResourceData = async (params: GETcourseResourceParams) => {
        const res = await axios.get((params.folderId ? GETcourseFolderResourcesApiUrl : GETcourseRootResourcesApiUrl)({
            ...params,
