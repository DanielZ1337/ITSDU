...
export function findMetaData(property: string, stringToSearch: string) {
	const regex = new RegExp(`<meta(?=.*?content="(.*?)")(?=[^>]*property="${property}").*?>`)
...
export function createQueryFunction<Params, Data>(
	...
	return function useQueryFunction(params: Params, queryConfig?: UseQueryOptions<Data, Error, Data, string[]>) {
