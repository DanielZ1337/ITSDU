...
export default function SearchResourcesDialog({ courseId }: { courseId: number }) {
		...
		<>
			<Button
				...
				className={cn(
					...
					sidebarActive ? 'lg:border-1 lg:py-2 lg:pr-12 lg:w-40 xl:w-52 justify-start' : 'w-full mx-auto justify-center'
				)}
				onClick={() => {
					setIsOpen(true)
					setSidebarActive(false)
			...
			>
				...
				<CommandList className={'overflow-hidden max-h-[50dvh]'}>
					<CommandEmpty className={cn(isFetching ? 'hidden' : 'py-6 text-center text-sm')}>
						No resources found.
					</CommandEmpty>
					{isFetching ? (
