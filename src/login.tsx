...
function Login() {
    const [organisations, setOrganisations] = useState<OrganisationsResponse | null>(null)
    const [search, setSearch] = useState('')

    const isSiteFiltered = organisations?.EntityArray.filter(org => org.IsSite)

    const filteredOrganisations = useCallback(() => {
        return isSiteFiltered?.filter(org => org.SiteName.toLowerCase().includes(search.toLowerCase()))
    ...
    useEffect(() => {
        fetch(new URL('/restapi/sites/all/organisations/v1?sitesFilter=itslearning', baseUrl).toString())
            .then(res => res.json())
    ...
    return (
        <div className='p-4 h-screen'>
            <Command className='h-full border' loop shouldFilter={false}>
                ...
                <CommandList className={cn('min-h-96 h-screen max-h-full', !memoizedOrganisations && 'flex flex-col items-center justify-center')}>
