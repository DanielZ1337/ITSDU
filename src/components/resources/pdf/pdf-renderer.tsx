...
export default function PdfRenderer({
	...
	const [intrinsicPageWidth, setIntrinsicPageWidth] = useState<number | null>(
		null,
	);
	const [intrinsicPageHeight, setIntrinsicPageHeight] = useState<number | null>(
		null,
	);
				...
				>
					...
					<div className="flex items-center gap-1">
						<Tooltip>
							...
							<TooltipContent side="bottom">
								Fit to page{" "}
								<kbd className="ml-1 text-[10px] opacity-60">Ctrl+0</kbd>
							</TooltipContent>
						</Tooltip>
									...
									>
										<Page
											...
											onRenderSuccess={(page) => {
												...
												setIntrinsicPageHeight(
													page.originalHeight || page.height,
												);
											}}
										/>
									</div>
	...
	return (
		<>
			<DropdownMenuItem
				...
				className={cn(fitMode === "page" && "bg-accent font-medium")}
			>
				Fit to page
			</DropdownMenuItem>
			<DropdownMenuItem
				onSelect={() => setScale(-2)} // -2 signals fit to width
				className={cn(fitMode === "width" && "bg-accent font-medium")}
			>
				Fit to width
			</DropdownMenuItem>
