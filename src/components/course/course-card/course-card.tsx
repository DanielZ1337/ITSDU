...
export default function CourseCard({ card }: { card: ItslearningRestApiEntitiesCourseCard }) {
	...
	const { mutate, isLoading } = usePUTcourseFavorite(
		...
		{
			onError(error) {
				toast.error(`An error occurred while trying to update the course's favorite status.`, {
					description: error.message || "An error occurred while trying to update the course's favorite status.",
				})
			},
			onSuccess() {
				card.IsFavouriteCourse = !card.IsFavouriteCourse
				toast.success(`Course ${card.Title} is now ${card.IsFavouriteCourse ? 'starred' : 'unstarred'}.`)
			},
		}
	)
			...
			>
				...
				>
					<div className={'flex flex-row justify-between'}>
						<span className={'leading-snug tracking-tighter text-black font-semibold text-balance'}>{card.Title}</span>
						<Button
							variant={'ghost'}
							size={'icon'}
							disabled={isLoading}
