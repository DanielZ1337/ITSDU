...
function ParseMarkdown({
  ...
  const components = {
    ...
    h4: ({ className, ...props }: ComponentTypes) => (
      <h4
        className={cn("text-md mt-8 scroll-m-20 font-medium tracking-tight", className)}
        {...props}
      />
    ),
    ...
    img: ({ className, alt, src, ...props }: ComponentTypes) => (
      <img
        className={cn(
          "w-full h-auto my-2 inline-flex rounded-md border object-contain",
          className,
        )}
        src={src}
        alt={alt}
    ...
    hr: ({ ...props }: ComponentTypes) => <hr className="my-4 md:my-8" {...props} />,
