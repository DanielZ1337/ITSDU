...
function IndexedDB() {
  ...
  let totalSize = useCallback(
    () =>
      allResources &&
      formatSize(allResources.reduce((acc, resource) => acc + resource.size, 0)),
    [allResources],
  )();
  let filteredTotalSize = useCallback(
    () => formatSize(filteredResources.reduce((acc, resource) => acc + resource.size, 0)),
    [filteredResources],
  )();
...
function DefaultSortTypeSetting() {
  const [defaultSortType, setDefaultSortType] = useState<"starred" | "unstarred" | "all">(
    "starred",
  );
...
function RefreshAccessTokenIntervalSetting() {
  ...
  const [refreshAccessTokenInterval, setRefreshAccessTokenInterval] = useState<number>(
    DEFAULT_REFRESH_ACCESS_TOKEN_INTERVAL / 60 / 1000,
  );
...
function SettingsCardSection({
  ...
  return (
    <div
      className={"p-4 md:p-8 bg-neutral-100 dark:bg-neutral-800 rounded-xl w-full h-max"}
    >
      <h1
        data-title={title}
        ref={ref}
