...
export default function Message({ message, children, role }: {
    ...
    return (
        <div className={cn("flex flex-row items-start justify-start py-3 overflow-x-hidden", role === "user" && 'flex-row-reverse')}>
            <div className="flex-shrink-0">
                <MessageAvatar role={role} />
            </div>
            <div className="mx-2 flex flex-col drop-shadow">
                <span className={cn("font-bold", role === "user" ? 'text-right' : 'text-left')}>{role === "user" ? "You" : "ITSDU AI"}</span>
                <span
