...
export default function AISidePanel({ elementId }: { elementId: string | number }) {
  ...
  const reversedPreviousMessages = useMemo(
    () =>
      previousMessages?.pages
        ?.map((page) => page.previousMessages)
        .flat()
        .sort((a, b) => {
  ...
  return (
    <div
      className={cn(
        "flex flex-col transition-all overflow-hidden max-h-full shrink-0 w-[33vw] border-l-4",
      )}
    >
      <div className="flex max-h-full flex-1 flex-col overflow-hidden p-4">
        <div className="flex max-h-full flex-1 flex-col rounded-md p-4 bg-foreground/10">
          ...
          <div className="flex max-h-full flex-1 flex-col overflow-hidden rounded-md p-4 bg-foreground/10 space-y-4">
            <div className="-ml-2 flex max-h-full flex-1 flex-col-reverse overflow-y-auto px-2">
              ...
              {reversedPreviousMessages &&
                reversedPreviousMessages.map((message, i) => (
                  <m.div
              ...
              {elementExistsLoading || uploadingDocument || isPreviousMessagesLoading ? (
                ...
                previousMessages &&
                previousMessages.pages.length > 0 &&
                previousMessages.pages[previousMessages.pages.length - 1].totalMessages >
                  previousMessages.pages[previousMessages.pages.length - 1].pageSize && (
                  <div ref={ref} className="flex flex-row items-center justify-center">
                    <span className="text-white/50">
                      {isFetchingNextPage
                        ? "Loading..."
                        : previousMessages.pages[previousMessages.pages.length - 1]
                              .totalMessages >
                            previousMessages.pages[previousMessages.pages.length - 1]
                              .pageIndex *
                              previousMessages.pages[previousMessages.pages.length - 1]
            ...
            >
              ...
              />
              <Button
                disabled={!elementExists || messageIsLoading || message.trim() === ""}
                type="submit"
                variant={"secondary"}
