...
export default function MessagesAddRecipients() {
	...
	useEffect(() => {
		...
		setFilteredRecipients(
			recipientsSearch.filter(
				(recipient) => !recipientsSelected.includes(recipient),
			),
		);
					...
					{recipientsSelected.length > 0 && (
						...
						>
							{recipientsSelected.map((recipient) => (
								<Button
									className={
										"justify-start shrink-0 animate-in slide-in-from-left-16 fade-in-5 py-7 inline-flex gap-3 bg-success/5 hover:bg-destructive/20"
									}
									key={recipient.Id}
									variant={"secondary"}
									onClick={() =>
