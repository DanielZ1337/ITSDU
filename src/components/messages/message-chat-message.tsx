...
export default function MessageChatMessage({
	...
	const isImage =
		attachmentUrl && attachmentName?.match(/\.(jpeg|jpg|gif|png)$/);
	...
	if (isSystemMessage)
		return (
			<p
				className={cn(
					"whitespace-pre-wrap break-all text-center text-gray-500 italic",
				)}
			>
	...
	return (
		<div className={cn("mb-4 flex flex-col")}>
			<span
				className={cn(
					"font-semibold flex text-sm text-foreground/80 mb-1",
					...
					<div
						className={cn(
							...
							me
								...
								: "float-left bg-foreground/10",
							isImage || isVideo
								? "max-w-[50dvw] max-h-[50dvh] bg-transparent"
								: "max-w-[80dvw]",
							(isDeletingMessage || isRestoringMessage) &&
					...
					>
						...
						{attachmentRef.current && isImage && (
							<Dialog>
								<DialogTrigger
									className={
										"max-w-[50dvw] max-h-[50dvh] rounded-md overflow-hidden"
									}
								>
									<img
										loading="lazy"
										...
										alt={attachmentName}
										className={
											"max-w-full h-full rounded-md hover:scale-105 transform transition-all duration-200"
										}
									/>
								</DialogTrigger>
								<DialogContent
								...
								>
									<div
										className={"rounded-md mx-auto w-fit mt-4 overflow-hidden"}
									>
										<img
											loading="lazy"
											src={attachmentRef.current}
											alt={attachmentName}
											className={
												"max-w-full h-full hover:scale-105 transform transition-all duration-200"
											}
										/>
									</div>
									<p
										className={
											"mt-0 sm:mt-2 xl:mt-4 xl:text-base text-gray-500 text-sm text-center"
										}
									>
										{attachmentName}
									</p>
