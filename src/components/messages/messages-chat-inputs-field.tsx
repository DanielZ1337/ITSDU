...
export default function MessagesChatInputsField() {
	...
	useEffect(() => {
		if (!isNewChat) return;
		const messageCanReply =
			!data?.pages[0].Messages.EntityArray[0].IsBroadcastMassMessage ||
			data?.pages[0].Messages.EntityArray[0].MessageThreadParticipants !== null;
		...
		<div className="flex max-h-64 flex-col border-t p-4">
			<form className="flex items-center gap-2" onSubmit={handleSubmit}>
				...
				<Button
					...
					type="submit"
					disabled={
						isSendingMessage || (message === "" && !files) || isSendingFile
					}
				>
