							...
							<CardDescription>
								...
								<div className='mt-2 flex gap-2 justify-end items-center w-full max-w-full overflow-auto transition-all'>
									...
									>
										...
										<span className='group-data-[state=on]:visible group-data-[state=on]:hidden'>Unsorted</span>
									</Toggle>
								</div>
							</CardDescription>
						</CardHeader>
										...
										Array.from(selectedDocuments.values()).map((document) => (
											...
											>
												<Card className='overflow-hidden bg-secondary border-none'>
													<CardHeader>
														<CardTitle className='truncate text-lg'>{document.Title}</CardTitle>
...
function CourseDocuments({
		...
		return (
			<div className='flex justify-center text-center flex-col gap-2'>
				<Loader
					...
					size={'sm'}
				/>
				<span className='text-muted-foreground text-sm text-center'>Loading documents...</span>
			</div>
		)
	}
	...
	const allDocumentsForCourseSelected =
		data.every((resource) => selectedDocuments?.has(resource.ElementId)) && data.length > 0
