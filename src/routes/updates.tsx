            ...
            <div className="flex flex-col gap-4">
                {updates && updates.pages.map((page) => (
                    page.EntityArray.map((update) => (
                        <Suspense fallback={<div>Loading...</div>} key={update.NotificationId}>
...
function NotificationCard({ notification }: { notification: ItslearningRestApiEntitiesStreamItemV2 }) {
    ...
    const [showLightBulletin, setShowLightBulletin] = React.useState(false)

    return (
        <div className="bg-foreground/10 rounded-md p-4">
            <h2 className="text-lg font-bold">
                {notification.ElementsCount === 0 ? <>New announcement available in <NotificationLocation notification={notification} /></> : notification.ElementsCount === 1 ? (
                    <>
                        {notification.PublishedBy.FullName} added <NotificationElement element={data!.EntityArray[0]} /> in <NotificationLocation notification={notification} />
                    </>
                ) : (
                    <>
                        {notification.PublishedBy.FullName} added <NotificationElement element={data!.EntityArray[0]} /> and <button className="text-blue-500 hover:text-blue-600 transition-colors hover:underline" onClick={() => setShowElements(prev => !prev)}>{notification.ElementsCount - 1} more</button> in <NotificationLocation notification={notification} />
                    </>
                )}
            </h2>
            {notification.LightBulletin && <button className="text-blue-500 hover:text-blue-600 transition-colors hover:underline" onClick={() => setShowLightBulletin(prev => !prev)}>{showLightBulletin ? "Hide" : "Show"} light bulletin</button>}
            <AnimatePresence>
                ...
                >
                    {data!.EntityArray.slice(1).map((element, i) => (
                        <motion.div key={element.ElementId} className="mt-2"
                            ...
                            animate={{ opacity: 1, y: 0, transition: { delay: i * 0.1, type: "tween", duration: 0.3 } }}
                            exit={{ opacity: 0, y: 10 }}
                        >
            ...
            {showLightBulletin && notification.LightBulletin && <div className="mt-4"><LightbulletinCard bulletin={notification.LightBulletin} /></div>}
        </div>
    )
}
...
function NotificationLocation({ notification }: { notification: ItslearningRestApiEntitiesStreamItemV2 }) {
    return (
        <Link className="text-blue-500 hover:text-blue-600 transition-colors hover:underline" to={`/courses/${notification.LocationId}`}>
