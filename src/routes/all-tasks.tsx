...
export default function TaskManager() {
	...
	const flatMapped = useMemo(() => tasks?.pages.flatMap((page) => page.EntityArray), [tasks])

	const filteredTasks = useMemo(() => {
		return flatMapped?.filter((task) => {
			if (filter === 'active') {
				return task.Status !== 'Completed' && (!task.Deadline || !isPast(new Date(task.Deadline)))
	...
	const getStatusInfo = (status: keyof typeof ItslearningRestApiEntitiesTaskStatus, isOverdue: boolean) => {
		if (isOverdue)
			return {
			...
			>
				...
				{filteredTasks?.map((task) => {
					...
					return (
						<Card
							key={task.TaskId}
							className={cn(isOverdue && 'border-destructive', 'h-full flex flex-col bg-foreground/10')}
						>
							<CardHeader>
							...
							<CardContent className='h-full flex flex-col justify-between'>
								<div className='h-full flex flex-col'>
									...
									<div className='flex items-center text-sm mt-2'>
										...
										<span>{task.Deadline ? format(new Date(task.Deadline), 'p') : 'No deadline'}</span>
									</div>
								</div>
								<Button
