                ...
                <ResizablePanelGroup autoSaveId={"course-index"} direction="horizontal">
                    ...
                    <TooltipProvider disableHoverableContent delayDuration={0}>
                        <Tooltip onOpenChange={setOpenTooltip} open={openTooltip && !longPress}>
                            <TooltipTrigger asChild>
                                <div onMouseUp={handleClick} onMouseDown={handleMouseDown}
                                     onTouchStart={handleMouseDown}
                                     className="py-40 hover:py-20 active:py-0 transition-all duration-1000 ease-in-out">
                                    ...
                                    >
                                        <div
                                            className="h-7 w-1 bg-white/50 rounded-full group-hover:bg-white/70 group-active:bg-white transition-all duration-200 ease-in-out"/>
                                    </ResizableHandle>
                                </div>
                            </TooltipTrigger>
                            <TooltipContent side="left">
                                <p className="text-sm text-white">Drag to resize</p>
                                <p className="text-sm text-white">Click
                                    to {ref.current?.isCollapsed() ? "expand" : "collapse"}</p>
                            </TooltipContent>
                        </Tooltip>
                    </TooltipProvider>
                    <ResizablePanel ref={ref} collapsible minSize={5} className={cn(!longPress && "transition-all")}>
                        <div
                            ...
                            <h2 className={"text-xl font-bold inline-flex gap-4 shrink-0"}><Files
                                className="shrink-0"/> Resources</h2>
                            <Suspense
