...
export async function openLinkInBrowser(url: string, sso: boolean = true) {
  if (sso) {
    const { data } = await axios.get(
      new URL("/restapi/personal/sso/url/v1", ITSLEARNING_URL()).toString(),
      {
        params: {
    ...
    async (_, elementId: string | number) => {
      try {
        ...
        const { downloadUrl, accessToken } =
          await getMicrosoftOfficeDocumentAccessTokenAndUrl(win);
        win.close();
  ...
  ipcMain.handle(
    ...
    async (
      ...
      const res = await fetch(
        new URL(`/api/checkFile/${elementId}`, baseUrl).toString(),
      );
      if (res.status === 200) throw new Error("File already exists for AI");
      if (res.status !== 404)
        throw new Error("Could not check if file exists for AI");
      ...
      const uploadRes = await axios.post(
        new URL(`/api/uploadFile/${elementId}`, baseUrl).toString(),
        {
  ...
  ipcMain.handle(
    ...
    async (event, { elementIds }: { elementIds: string[] }) => {
      try {
        ...
        const Dialog = await import("electron").then(
        ...
        const loadedPdfs = await Promise.all(
          elementIds.map(async (elementId) => {
            ...
            const resourceByElementId =
              await getResourceLinkByElementID(elementId);
    ...
    > => {
      try {
        ...
        const downloadPromises = elementIds.map(async (elementId) => {
          ...
          const saveFilename = decodeURIComponent(
            match ?? `file_${elementId}.bin`,
          );
          ...
          if (options.organizeByCourse) {
            ...
            return new Promise<void>((resolve, reject) => {
              ...
              stream.on("end", () => {
                courses[courseName].file(saveFilename, Buffer.concat(chunks), {
                  binary: true,
                });
                resolve();
              });
...
function getPlannerPayloadUrl(courseId: string | number) {
  const payloadUrl = new URL(
    `https://sdu.itslearning.com/Planner/Planner.aspx`,
  );
...
async function getCoursePlansInformation(body: string) {
  ...
  courses.each((_, element) => {
    ...
    // Extract plans count
    const plansText = $(element)
      .find(".itsl-topic-expander .itsl-topic-expanded-text")
      .text()
      .trim();
    ...
    const datesText = $(element)
      .find(".itsl-topic-expander .itsl-topic-dates")
      .text()
      .trim();
...
async function getCoursePlansElements(html: string) {
  ...
  gridRows.each((_, element) => {
    ...
    const title = row
      .find(".itsl-planner-min-title-width .itsl-plan-title-label")
      .text()
      .trim();
    const dateString = row
      .find(".itsl-plan-date .itsl-inline-date-picker-view")
      .text()
      .trim();
    ...
    resourceActivityElements.each((_, element) => {
      ...
      const link = resourceActivity
        .find("a.itsl-plan-elements-item-link")
        .attr("href");
